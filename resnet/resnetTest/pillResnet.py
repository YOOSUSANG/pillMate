...
def predict(criterion, net, device, realtest_loader, classes):
    ...
    total = 0
    idx = 1
    # 예측 성공한 알약 이름을 저장할 경로 267줄이랑 똑같은 경로입니다.
    file_path = ''
    # 업로드된 이미지가 하나이기 떄문에 이미 이전에 인식한 알약을 지워줘야합니다.
    try:
        os.remove(file_path)
    ...
    for batch_idx, (inputs, targets) in enumerate(realtest_loader):
        ...
        for i in range(targets.size(0)):
            ...
            idx+=1
            # 알약 예측 성공시 알약 이름을 ''에 경로에 넣어서 리액트가 인식해도록 해야합니다. 
            with open('', 'w', encoding='utf-8') as log_file:
